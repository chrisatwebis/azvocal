 ...
 */
function dp_dbsite_install_tasks_alter(&$tasks, &$install_state) {
  $new_tasks = array(
...
function md_install_selected_profile() {
  ...
  if ($driver == 'mysql') {
    ...
    $file_name = dirname(__FILE__) . '/5.support2/5.support2.sql';
	if(!file_exists($file_name)) {
	  $file_name = dirname(__FILE__) . '/5.support2.sql';
	}
    if(!file_exists($file_name)) {
	  $file_name = DRUPAL_ROOT . '/_resource/5.support2.sql';
	}
